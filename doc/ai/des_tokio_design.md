# Deterministic Async Runtime + `descartes_tokio` Design
...
- Provide a Tokio-like facade crate/module: **`descartes_tokio`**.
  - `descartes_tokio::task::spawn` requires `Send` futures.
  - `descartes_tokio::task::spawn_local` exists for `!Send`.
  - `descartes_tokio::task::JoinHandle` detaches on drop (Tokio-like); cancellation is explicit via `abort()`.
...
- Provide Tokio-like time: **`descartes_tokio::time::Instant`**.
...
- `doc/ai/descartes_event_scheduling_policies.md`
- `doc/ai/descartes_tokio_concurrency_replay.md`
...
- `des-core/src/async_runtime.rs` implements `DesRuntime` as a `Component`.
  ...
  - Uses `create_descartes_waker(...)` → `defer_wake(...)`.
...
- When using `tokio::select!` in `descartes_tokio` tasks, prefer `biased;` to keep stable polling order (Tokio defaults to randomizing branch poll order for fairness).

## `descartes_tokio` Surface (MVP)

### `descartes_tokio::runtime`
- `Runtime::new()`
...
1. **Step 1 (next):** Simplify `DesRuntime` polling context
   ...
   - Time reads use `descartes_core::scheduler::current_time()`.
...
4. **Step 4:** Introduce `descartes_tokio` MVP
...
- 2026-01-10: Step 1 completed: removed duplicated time/scheduler TLS from `des-core/src/async_runtime.rs` and routed time reads through `descartes_core::scheduler::current_time()`.
...
- 2026-01-10: Implemented `descartes_tokio::time::Instant` parity improvements (checked/saturating/strict duration_since) and added `des-tokio/tests/time_parity.rs`.
- 2026-01-10: Implemented `descartes_tokio::time::timeout` (+ `Elapsed` and `Timeout`) and added `des-tokio/tests/timeout.rs`.
- 2026-01-10: Updated `descartes_tokio::task::JoinHandle` to Tokio-like detach-on-drop semantics. Added `abort_cancels_task` and updated the old drop-cancels test to `drop_detaches_task`.
- 2026-01-10: Added `descartes_tokio::sync::oneshot` and `des-tokio/tests/oneshot.rs` covering send/recv, sender drop, and receiver drop.
- 2026-01-10: Added `descartes_tokio::sync::mpsc` (bounded channel) with `send/try_send` + `recv/try_recv` and deterministic FIFO wake ordering for blocked senders. Added `des-tokio/tests/mpsc.rs`.
- 2026-01-10: Added `descartes_tokio::sync::Notify` with FIFO waiter order and Tokio-like permit semantics. Fixed `notify_one/notify_waiters` to mint permits when waking existing waiters so `notified().await` completes. Added `des-tokio/tests/notify.rs`.
