...
/// often the dependent-call cascade triggers the terminal predicate under random seeds.
#[test]
#[ignore]
...
/// prefix checkpointing + replay in a multi-component model.
#[test]
#[ignore]
fn splitting_success_rate_variability() {
    ...
    for run in 0..5u64 {
        ...
        let est = estimate_with_splitting(
            cfg.clone(),
            SimulationConfig { seed: base_seed },
            format!("dep_calls_splitting_run{}", run),
            setup_splitting,
            |s| s.metastable,
        )
...
fn comprehensive_metastability_comparison() {
    ...
    let mc = estimate_monte_carlo(
        ...
        "dep_calls_monte_carlo".to_string(),
        setup_naive,
        |s| s.metastable,
    )
    .expect("monte carlo estimate should succeed");
