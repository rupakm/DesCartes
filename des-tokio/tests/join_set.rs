...
fn join_set_returns_results_in_completion_order() {
    ...
    descartes_tokio::task::spawn_local(async move {
        let mut set = descartes_tokio::task::JoinSet::new();
...
fn join_set_drop_aborts_tasks() {
    ...
    descartes_tokio::task::spawn_local(async move {
        ...
        set.spawn(async move {
            descartes_tokio::time::sleep(Duration::from_millis(100)).await;
        ...
        descartes_tokio::time::sleep(Duration::from_millis(200)).await;
    });
...
fn join_set_abort_handle_cancels_task() {
    ...
    let out: Arc<Mutex<Vec<Result<u64, descartes_tokio::task::JoinError>>>> =
    ...
    assert_eq!(v.len(), 1);
    assert!(matches!(v[0], Err(descartes_tokio::task::JoinError::Cancelled)));
}
