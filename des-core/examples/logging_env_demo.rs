    ...
    fn process_event(
        ...
        match event {
            SimpleEvent::Tick => {
                ...
                info!(counter = self.counter, "{} Component tick #{}", scheduler.time(), self.counter);
            ...
            SimpleEvent::Tock => {
                ...
                info!(counter = self.counter, "{} Component tock #{}", scheduler.time(), self.counter);
            }
        }
    }
}
...
fn main() {
    ...
    let final_component = sim.remove_component::<SimpleEvent, SimpleComponent>(component_key).unwrap();
    
    info!(final_counter = final_component.counter, "Simulation completed");
