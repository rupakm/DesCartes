...
fn test_task_component_async_integration() {
    ...
    runtime.spawn(async move {
        log_clone.lock().unwrap().push("Async: Task 1 started".to_string());
        sim_sleep(Duration::from_millis(25)).await;
        log_clone.lock().unwrap().push("Async: Task 1 after 25ms".to_string());
        sim_sleep(Duration::from_millis(25)).await;
        log_clone.lock().unwrap().push("Async: Task 1 completed".to_string());
    });
    
    let log_clone2 = execution_log.clone();
    runtime.spawn(async move {
        log_clone2.lock().unwrap().push("Async: Task 2 started".to_string());
        sim_sleep(Duration::from_millis(75)).await;
        log_clone2.lock().unwrap().push("Async: Task 2 completed".to_string());
    });
                    ...
                    move |scheduler_inner| {
                        ...
                        if counter < 3 {
                            ...
                            scheduler_inner.schedule_closure(
                                SimTime::from_duration(Duration::from_millis(30)),
                                move |_| {
                                    log_clone_inner2.lock().unwrap().push("Task: Periodic task #3".to_string());
                                }
                            );
                        }
                    }
