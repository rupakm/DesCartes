...
fn basic_metrics_example() {
    ...
    println!(
        "      - API Success Requests: {}",
        metrics
            .get_counter("requests", &[("service", "api"), ("status", "success")])
            .unwrap_or(0)
    );
    println!(
        "      - API Error Requests: {}",
        metrics
            .get_counter("requests", &[("service", "api"), ("status", "error")])
            .unwrap_or(0)
    );
    println!(
        "      - Bytes Sent: {}",
        metrics
            .get_counter("bytes_sent", &[("service", "api")])
            .unwrap_or(0)
    );
    ...
    println!(
        "      - CPU Usage: {:.1}%",
        metrics
            .get_gauge("cpu_usage", &[("host", "server1")])
            .unwrap_or(0.0)
    );
    println!(
        "      - Memory Usage: {:.1}%",
        metrics
            .get_gauge("memory_usage", &[("host", "server1")])
            .unwrap_or(0.0)
    );
    println!(
        "      - Queue Depth: {}",
        metrics
            .get_gauge("queue_depth", &[("service", "api")])
...
fn client_metrics_example() {
    ...
    let requests_sent = metrics
        .get_counter("requests_sent", &[("component", "metrics-client")])
        .unwrap_or(0);
    let total_requests_gauge = metrics
        .get_gauge("total_requests", &[("component", "metrics-client")])
        .unwrap_or(0.0);

    println!("      - Requests Counter: {}", requests_sent);
    println!(
        "      - Total Requests Gauge: {}",
        total_requests_gauge as u64
    );
    ...
    if let Some(attempts_sent) =
        metrics.get_counter("attempts_sent", &[("component", "metrics-client")])
    {
    ...
    if let Some(successes) =
        metrics.get_counter("responses_success", &[("component", "metrics-client")])
    {
        println!("      - Successful Responses: {}", successes);
    }

    if let Some(failures) =
        metrics.get_counter("responses_failure", &[("component", "metrics-client")])
    {
        println!("      - Failed Responses: {}", failures);
    }

    if let Some(timeouts) =
        metrics.get_counter("requests_timeout", &[("component", "metrics-client")])
    {
...
fn comprehensive_analysis_example() {
    ...
    println!(
        "      - Total Histogram Metrics: {}",
        snapshot.histograms.len()
    );

    // List all unique metric names
    let counter_names: std::collections::HashSet<_> =
        snapshot.counters.iter().map(|(name, _, _)| name).collect();
    let gauge_names: std::collections::HashSet<_> =
        snapshot.gauges.iter().map(|(name, _, _)| name).collect();
    let histogram_names: std::collections::HashSet<_> = snapshot
        .histograms
        .iter()
        .map(|(name, _, _)| name)
        .collect();
