...
fn periodic_task_example() {
    ...
    let periodic_task = PeriodicTask::with_count(
        move |scheduler| {
            ...
            println!("   🔄 PeriodicTask execution #{} at {:?}", count, scheduler.time());
        },
    ...
    println!("   ✅ Task executed {} times (should be 3)\n", execution_count.load(Ordering::Relaxed));
}
...
fn task_coordination_example() {
    ...
    let incrementer = PeriodicTask::with_count(
        move |scheduler| {
            ...
            *count += 1;
            println!("   📈 Incrementer: count = {} at {:?}", *count, scheduler.time());
        },
    ...
    let reporter = TimeoutTask::new(move |scheduler| {
        ...
        println!("   📊 Reporter: final count = {} at {:?}", *count, scheduler.time());
    });
    ...
    let resetter = TimeoutTask::new(move |scheduler| {
        ...
        let old_count = *count;
        *count = 0;
        println!("   🔄 Resetter: reset count from {} to 0 at {:?}", old_count, scheduler.time());
    });
