...
fn exponential_backoff_example() {
    ...
    assert_eq!(
        metrics.get_counter("requests_sent", &component_label),
        Some(3)
    );
    assert_eq!(
        metrics.get_gauge("total_requests", &component_label),
        Some(3.0)
    );

    // Should have some attempts (at least the original requests)
    let attempts_sent = metrics
        .get_counter("attempts_sent", &component_label)
        .unwrap_or(0);
    assert!(
        attempts_sent >= 3,
        "Should have at least 3 attempts (original requests), got {}",
        attempts_sent
    );

    // Check for successful responses (requests are actually succeeding)
    let successes = metrics
        .get_counter("responses_success", &component_label)
        .unwrap_or(0);
    let failures = metrics
        .get_counter("responses_failure", &component_label)
        .unwrap_or(0);
    let timeouts = metrics
        .get_counter("requests_timeout", &component_label)
        .unwrap_or(0);

    // Should have some responses (either success or failure)
    assert!(
        successes + failures + timeouts > 0,
        "Should have some responses or timeouts"
    );

    // If requests are succeeding, we might not have retries
    let retries_scheduled = metrics
        .get_counter("retries_scheduled", &component_label)
        .unwrap_or(0);
...
fn token_bucket_example() {
    ...
    let client = sim
        .remove_component::<ClientEvent, SimpleClient<TokenBucketRetryPolicy>>(client_id)
        .unwrap();
    let metrics = client.get_metrics();
    ...
    assert!(
        attempts_sent >= 4,
        "Should have at least 4 attempts (original requests), got {}",
        attempts_sent
    );
...
fn success_based_example() {
    ...
    // Create a server first
    let server = Server::with_constant_service_time(
        "success-based-server".to_string(),
        1,
        Duration::from_millis(100),
    );
    let server_id = sim.add_component(server);
    ...
    let client = sim
        .remove_component::<ClientEvent, SimpleClient<SuccessBasedRetryPolicy>>(client_id)
        .unwrap();
    let metrics = client.get_metrics();
    ...
    assert!(
        attempts_sent >= 5,
        "Should have at least 5 attempts (original requests), got {}",
        attempts_sent
    );
    ...
    let retries_scheduled = metrics
        .get_counter("retries_scheduled", &component_label)
        .unwrap_or(0);
    let permanently_failed = metrics
        .get_counter("requests_failed_permanently", &component_label)
        .unwrap_or(0);
...
fn policy_comparison_example() {
    ...
    let exp_server =
        Server::with_constant_service_time("exp-server".to_string(), 1, Duration::from_millis(100));
    let exp_server_id = sim.add_component(exp_server);

    let token_server = Server::with_constant_service_time(
    ...
    let success_server = Server::with_constant_service_time(
        "success-server".to_string(),
        1,
        Duration::from_millis(100),
    );
    ...
    for (client_id, delay_offset) in [(exp_id, 0), (token_id, 100), (success_id, 200)] {
        ...
        sim.schedule_task(
            SimTime::from_duration(Duration::from_millis(delay_offset)),
            task,
        );
    }
    ...
    // Collect results
    let exp_client = sim
        .remove_component::<ClientEvent, SimpleClient<ExponentialBackoffPolicy>>(exp_id)
        .unwrap();
    let token_client = sim
        .remove_component::<ClientEvent, SimpleClient<TokenBucketRetryPolicy>>(token_id)
        .unwrap();
    let success_client = sim
        .remove_component::<ClientEvent, SimpleClient<SuccessBasedRetryPolicy>>(success_id)
        .unwrap();
   ...
   |-----------------|----------|--------|--------------|"
    ...
    for (name, client_requests, active_requests, max_attempts) in [
        (
            "Exponential",
            exp_client.requests_sent,
            exp_client.active_requests.len(),
            exp_client.retry_policy.max_attempts(),
        ),
        (
            "Token Bucket",
            token_client.requests_sent,
            token_client.active_requests.len(),
            token_client.retry_policy.max_attempts(),
        ),
        (
            "Success-Based",
            success_client.requests_sent,
            success_client.active_requests.len(),
            success_client.retry_policy.max_attempts(),
        ),
    ] {
        println!(
            "   | {:15} | {:8} | {:6} | {:12} |",
            name, client_requests, active_requests, max_attempts,
        );
    }

    // Assert that all clients sent their expected number of requests
    assert_eq!(
        exp_client.requests_sent, 3,
        "Exponential client should have sent 3 requests"
    );
    assert_eq!(
        token_client.requests_sent, 3,
        "Token bucket client should have sent 3 requests"
    );
    assert_eq!(
        success_client.requests_sent, 3,
        "Success-based client should have sent 3 requests"
    );
    ...
    assert_eq!(
        exp_metrics.get_counter("requests_sent", &exp_label),
        Some(3)
    );
    assert_eq!(
        token_metrics.get_counter("requests_sent", &token_label),
        Some(3)
    );
    assert_eq!(
        success_metrics.get_counter("requests_sent", &success_label),
        Some(3)
    );

    // All should have some attempts (at least the original requests)
    let exp_attempts = exp_metrics
        .get_counter("attempts_sent", &exp_label)
        .unwrap_or(0);
    let token_attempts = token_metrics
        .get_counter("attempts_sent", &token_label)
        .unwrap_or(0);
    let success_attempts = success_metrics
        .get_counter("attempts_sent", &success_label)
        .unwrap_or(0);

    assert!(
        exp_attempts >= 3,
        "Exponential client should have at least 3 attempts"
    );
    assert!(
        token_attempts >= 3,
        "Token bucket client should have at least 3 attempts"
    );
    assert!(
        success_attempts >= 3,
        "Success-based client should have at least 3 attempts"
    );
