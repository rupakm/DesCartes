...
use des_components::{
    ClientEvent, DesLoadBalanceStrategy, DesLoadBalancer, DesServiceBuilder, FifoQueue, Request,
    RequestAttempt, RequestAttemptId, RequestId, Server, ServerEvent,
};
use des_core::{Component, Execute, Key, Scheduler, SimTime, Simulation};
...
impl ExponentialClient {
    fn new(name: String, server_key: Key<ServerEvent>, rate: f64, total_requests: usize) -> Self {
            ...
            ClientEvent::ResponseReceived { response } => {
                ...
                // Update metrics
                {
                    ...
                    match metrics.min_latency {
                        ...
                        Some(min) if latency_duration < min => {
                            metrics.min_latency = Some(latency_duration)
                        }
                        _ => {}
                    }
                    ...
                    match metrics.max_latency {
                        ...
                        Some(max) if latency_duration > max => {
                            metrics.max_latency = Some(latency_duration)
                        }
                        _ => {}
                    }
                }
...
fn test_exponential_traffic_single_server_with_queue() {
    ...
    let final_client = simulation
        .remove_component::<ClientEvent, ExponentialClient>(client_key)
        .unwrap();
    ...
    if metrics.responses_received > 0 {
        ...
        assert!(
            ...
            "Average latency should be at least service time"
        );
    }
    ...
    println!(
        ...
        final_server.requests_processed
    );
...
fn test_exponential_traffic_load_balancer_three_servers() {
                ...
                ClientEvent::SendRequest => {
                    if self.sent_requests < self.total_requests {
                        let request = Request::new(
                            RequestId(self.next_request_id),
                            scheduler.time(),
                            format!("LB Request {} from {}", self.next_request_id, self.name)
                                .into_bytes(),
                        );
    ...
    // Get final metrics
    let final_lb_client = simulation
        .remove_component::<ClientEvent, LoadBalancingClient>(lb_client_key)
        .unwrap();
    ...
    if lb_metrics.responses_received > 0 {
        ...
        println!(
            "Min latency: {:?}",
            lb_metrics.min_latency.unwrap_or_default()
        );
        println!(
            "Max latency: {:?}",
            lb_metrics.max_latency.unwrap_or_default()
        );

        // Verify we got reasonable results
        assert!(lb_metrics.requests_sent > 0, "Should have sent requests");
        assert!(
            lb_metrics.responses_received > 0,
            "Should have received responses"
        );
        assert!(
            avg_latency >= Duration::from_millis(80),
            "Average latency should be at least min service time"
        );
    }

    // Get metrics from each server
    let final_server1 = simulation
        .remove_component::<ServerEvent, Server>(server1_key)
        .unwrap();
    let final_server2 = simulation
        .remove_component::<ServerEvent, Server>(server2_key)
        .unwrap();
    let final_server3 = simulation
        .remove_component::<ServerEvent, Server>(server3_key)
        .unwrap();

    println!(
        "Server 1 processed: {} requests",
        final_server1.requests_processed
    );

    println!(
        "Server 2 processed: {} requests",
        final_server2.requests_processed
    );

    println!(
        "Server 3 processed: {} requests",
        final_server3.requests_processed
    );
    // Note: With the new metrics system, metrics are recorded globally
}
