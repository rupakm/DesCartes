...
fn test_periodic_task_behavior() {
    ...
    assert_eq!(
        ...
        "Client should have sent exactly 5 requests"
    );

    println!(
        "PeriodicTask sent {} requests as expected",
...
fn test_periodic_task_automatic_termination() {
    ...
    assert_eq!(
        final_count, 3,
        "PeriodicTask should have executed exactly 3 times"
    );
...
fn test_task_cleanup_edge_cases() {
    ...
    assert!(
        ...
        "TimeoutTask should have fired immediately"
    );
    ...
    assert!(
        ...
        "ClosureTask should have executed"
    );
    ...
    assert_eq!(
        ...
        5,
        "All 5 TimeoutTasks should have executed"
    );
...
fn test_task_timing_precision() {
    ...
    for (i, &expected_delay) in expected_times.iter().enumerate() {
        ...
        assert_eq!(
            ...
            "Task {i} should execute at precise time"
        );
    }
...
fn test_task_error_handling() {
    ...
    for i in 0..3 {
        ...
        sim.schedule_task(SimTime::from_duration(Duration::from_millis(i * 10)), task);
    }
...
fn test_mixed_task_integration() {
    ...
    sim.schedule_task(
        SimTime::from_duration(Duration::from_millis(45)),
        timeout_task,
    );
    ...
    assert_eq!(
        final_results.len(),
        5,
        "Should have 5 total executions (3 periodic + 1 timeout + 1 closure)"
    );
    ...
    let time_0_results: Vec<_> = final_results
        .iter()
        .filter(|r| r.contains("at SimTime(0)"))
        .collect();
    assert_eq!(
        time_0_results.len(),
        2,
        "Should have 2 tasks executing at time 0"
    );
    assert!(
        final_results
            .iter()
            .any(|r| r.contains("Timeout at SimTime(45000000)")),
        "Timeout should execute at 45ms"
    );
